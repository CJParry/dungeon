...
public final class CommandSets implements Serializable {
  ...
  private CommandSet initializeDefaultCommandSet() {
    ...
    commandSet.addCommand(new Command("achievements", "Displays the already unlocked achievements.") {
      ...
      public void execute(@NotNull String[] arguments) {
        AchievementTrackerWriter.parseCommand(arguments, game.getGameState());
      }
    });
    ...
    commandSet.addCommand(new Command("map", "Shows a map of your surroundings.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        WorldMapWriter.writeMap(game.getGameState());
      }
    });
  ...
  private CommandSet initializeDebugCommandSet() {
    ...
    commandSet.addCommand(new Command("map", "Produces a map as complete as possible.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        WorldMapWriter.writeDebugMap(game.getGameState());
      }
    });
      ...
      public void execute(@NotNull String[] arguments) {
        if (arguments.length != 0) {
          for (String argument : arguments) {
            ...
            World world = game.getGameState().getWorld();
            Creature creature = world.getCreatureFactory().makeCreature(givenId, world, game.getGameState());
            if (creature != null) {
