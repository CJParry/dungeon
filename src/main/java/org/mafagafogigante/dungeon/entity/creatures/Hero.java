...
import org.mafagafogigante.dungeon.entity.WeaponEnchantment;
...
public class Hero extends Creature {
  ...
  private final long serialVersionUID = Version.MAJOR;
  ...
  private static int DREAM_DURATION_IN_SECONDS = 4 * DungeonMath.safeCastLongToInteger(HOUR.as(SECOND));
  private final int MILLISECONDS_TO_SLEEP_AN_HOUR = 500;
  private final int MILLISECONDS_TO_FISH = 250;
  private final int SECONDS_TO_PICK_UP_AN_ITEM = 10;
  private final int SECONDS_TO_HIT_AN_ITEM = 4;
  private final int SECONDS_TO_EAT_AN_ITEM = 30;
  private final int SECONDS_TO_DRINK_AN_ITEM = 10;
  private final int SECONDS_TO_DROP_AN_ITEM = 2;
  private final int SECONDS_TO_UNEQUIP = 4;
  private final int SECONDS_TO_EQUIP = 6;
  private final int SECONDS_TO_FISH = 60;
  private final int SECONDS_TO_MILK_A_CREATURE = 45;
  private final int SECONDS_TO_READ_EQUIPPED_CLOCK = 4;
  private final int SECONDS_TO_READ_UNEQUIPPED_CLOCK = 10;
  private final double MAXIMUM_HEALTH_THROUGH_REST = 0.6;
  private final int SECONDS_TO_REGENERATE_FULL_HEALTH = 30000; // 500 minutes (or 8 hours and 20 minutes).
  private final int MILK_NUTRITION = 12;
  private final Id FISH_ID = new Id("FISH");
  private final Id WELL_FED_ID = new Id("WELL_FED");
  private final List<String> WELL_FED_LIST = Collections.emptyList();
  private final Effect WELL_FED_EFFECT = EffectFactory.getDefaultFactory().getEffect(WELL_FED_ID, WELL_FED_LIST);
  private final Walker walker = new Walker();
  private final Observer observer = new Observer(this);
  private final HeroSpellcaster spellcaster = new HeroSpellcaster(this);
  ...
  private int nextRandomTimeChunk() {
  ...
  public HeroSpellcaster getSpellcaster() {
    return spellcaster;
  }
   ...
   */
  public void rest(Engine engine) {
   ...
   */
  public void sleep(Engine engine) {
    int seconds;
   ...
   */
  public void look(GameState gameState) {
    observer.look(gameState);
  }
   ...
   */
  public void attackTarget(String[] arguments, Engine engine) {
   ...
   */
  public void pickItems(String[] arguments, Engine engine) {
    if (canSeeAnItem()) {
   ...
   */
  public void dropItems(String[] arguments, Engine engine) {
    List<Item> selectedItems = selectInventoryItems(arguments);
    for (Item item : selectedItems) {
      if (item == getWeapon()) {
   ...
   */
  public void eatItem(String[] arguments, Game game) {
    Item selectedItem = selectInventoryItem(arguments);
    if (selectedItem != null) {
      if (selectedItem.hasTag(Item.Tag.FOOD)) {
        game.getEngine().rollDateAndRefresh(SECONDS_TO_EAT_AN_ITEM);
   ...
   */
  public void fish(Engine engine) {
   ...
   */
  public void drinkItem(String[] arguments, Game game) {
    Item selectedItem = selectInventoryItem(arguments);
    if (selectedItem != null) {
      if (selectedItem.hasTag(Item.Tag.DRINKABLE)) {
        game.getEngine().rollDateAndRefresh(SECONDS_TO_DRINK_AN_ITEM);
        if (getInventory().hasItem(selectedItem)) {
   ...
   */
  public void parseMilk(String[] arguments, Engine engine) {
    if (arguments.length != 0) { // Specified which creature to milk from.
      ...
      if (milkableCreatures.isEmpty()) {
        ...
        if (Matches.fromCollection(milkableCreatures).getDifferentNames() == 1) {
          milk(milkableCreatures.get(0), engine);
        } else {
  ...
  public void examineItem(String[] arguments) {
    ...
    if (selectedItem != null) {
      ...
      if (selectedItem.getWeaponComponent() != null) {
        List<WeaponEnchantment> enchantments = selectedItem.getWeaponComponent().getEnchantments();
  ...
  private void milk(Creature creature, Engine engine) {
   ...
   */
  public void readItem(String[] arguments, Engine engine) {
    Item selectedItem = selectInventoryItem(arguments);
    if (selectedItem != null) {
   ...
   */
  private void learnSpell(@NotNull BookComponent book, Engine engine) {
    if (!book.isDidactic()) {
      throw new IllegalArgumentException("book should be didactic.");
    }
    SpellData spellData = new SpellData(engine);
  ...
  private void destroyItem(@NotNull Item target, Game game) {
    ...
    if (target.isBroken()) {
      ...
      while (getLocation().getInventory().hasItem(target) && !target.isBroken()) {
        ...
        game.getEngine().rollDateAndRefresh(SECONDS_TO_HIT_AN_ITEM);
      }
   ...
   */
  public void unequipWeapon(Engine engine) {
    if (hasWeapon()) {
   ...
   */
  public void printAllStatus(GameState gameState) {
    DungeonString string = new DungeonString();
    ...
    string.append(getAgeString(gameState));
    string.append(" old");
    string.append(".\n");
   ...
   */
  public void printAge(GameState gameState) {
    Writer.write(new DungeonString("You are " + getAgeString(gameState) + " old.", Color.CYAN));
  }

  private String getAgeString(GameState gameState) {
    return new Duration(dateOfBirth, gameState.getWorld().getWorldDate()).toString();
  }

  /**
   * Makes the Hero read the current date and time as well as he can.
   */
  public void readTime(Game game) {
    Item clock = getBestClock(game.getEngine());
    if (clock != null) {
      ...
      game.getEngine().rollDateAndRefresh(getTimeToReadFromClock(clock));
    }
    World world = getLocation().getWorld();
   ...
   */
  public void walk(String[] arguments, Game game) {
    walker.parseHeroWalk(arguments, game);
  }
   ...
   */
  @Nullable
  private Item getBestClock(Engine engine) {
    Item clock = null;
